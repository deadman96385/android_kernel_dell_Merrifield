...
main() {
    ...
    if [ "$EXTERNAL_MODULE_DIRECTORY" ]; then
        ...
        ./stitch.py --kernel-cmdline out/target/product/boot_cmdline --bootstub out/target/product/bootstub --kernel out/target/product/kernel --ramdisk out/target/product/ramdisk.img --output out/target/product/boot.unsigned

        ./gen_os --input out/target/product/boot.unsigned --output out/target/product/boot.img --xml MOS_OTA.XML --sign-with xfstk_no_xml
    fi
    run_depmod
    exit 0
}
